...
public:
	...
	bool PairInRange(const std::pair<int, int> min, const std::pair<int, int> max, const std::pair<int, int> value);
	std::pair<int,int> GetStartingCoord(const std::pair<int, int> coord, const std::pair<int,int> direction);
	void AdvanceCoordInDirection(std::pair<int, int>& coord, const std::pair<int, int> direction);
};